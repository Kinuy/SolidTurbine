...
int main(int /*argc*/, char **argv)
{
    ...
    try
    {
        ...
        // ── 3. DXF blade geometry export ──────────────────────────────────────
        //
        // createBladeInterpolator() is called separately here so DXFBlade3D
        // can take ownership without affecting the TurbineGeometry below.
        // config still holds all raw data so a second interpolator is cheap.
        //
        {
            ...
            auto dxfBlade3D = std::make_unique<DXFBlade3D>(std::move(dxfInterpolator));

            if (fs::exists("blade3D.dxf"))
                fs::rename("blade3D.dxf", dxf_path);

            // Write tab-separated blade section text file
        ...
        printTiming(3, "Blade DXF exported", t2, t3, "2 files");

        // ── 4. Build TurbineGeometry ──────────────────────────────────────────
        //
        // Fresh interpolator — independent from the one DXFBlade3D consumed.
        //
